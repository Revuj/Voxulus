...
class EasySeeso {
  ...
  async init(licenseKey, afterInitialized, afterFailed, userStatusOption) {
    await this.seeso.initialize(licenseKey, userStatusOption).then(
      function (errCode) {
        if (errCode === InitializationErrorType.ERROR_NONE) {
          afterInitialized();
          this.onCalibrationFinishedBind =
  ...
  startCalibration(
    ...
    const isStart = this.seeso.startCalibration(
      calibrationPoints,
      CalibrationAccuracyCriteria.Default
    );
    if (isStart) {
      ...
      this.seeso.removeCalibrationNextPointCallback(
        this.onCalibrationNextPoint
      );
  ...
  static openCalibrationPageQuickStart(
    licenseKey,
    userId,
    redirectUrl,
    calibraitonPoint
  ) {
    Seeso.openCalibrationPageQuickStart(
      licenseKey,
      userId,
      redirectUrl,
      calibraitonPoint
